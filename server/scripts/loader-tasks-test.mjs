...
const group = { id: 1, name: 'Group', headcount: 10, ration: { id: 2, name: 'Ration', isActive: true, feedingsPerDay: 2,
...
assert.equal(buildLoaderPlan({ ...group, ration: { ...group.ration, isActive: false } }), null)
assert.equal(buildLoaderPlan({ ...group, isActive: false }), null)
const plan = buildLoaderPlan(group)
assert.deepEqual(plan.steps.map(s => s.targetKg), [100, 50])
...
const prisma = { livestockGroup: { findMany: async () => [group, { ...group, id: 99, ration: { ...group.ration, isActive: false } }], findUnique: async () => group } }
...
try {
  ...
  assert.equal((await (await fetch(base + '/groups')).json()).groups.length, 1, 'Hide inactive rations')
  ...
  group.ration.isActive = false
  assert.equal((await post('/tasks', { ...newBody, id: randomUUID() })).status, 400)
  assert.deepEqual((await (await post('/tasks', newBody)).json()).task, created.task, 'Existing tasks survive deactivation')
