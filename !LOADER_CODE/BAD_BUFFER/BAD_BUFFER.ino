...
bool appendToQueue(const String& payload) {
  ...
  // If append still fails, try evicting oldest rows and retry a few times.
...
void flushQueueIfPossible() {
  ...
  unsigned long now = millis();
  if (liveSendBlockedUntilMs != 0 && now < liveSendBlockedUntilMs) return;
  ...
  if (sdReady && SD.exists(QUEUE_FILE)) {
    ...
    if (readFirstQueuedRows(batchJson, FLUSH_BATCH_MAX, batchRows)) {
      if (!sendPayload(batchJson)) {
        failed = true;
      } else if (!rewriteQueueSkippingFirstRows(batchRows)) {
        failed = true;
      } else {
        sentSd = batchRows;
  ...
  if (!failed && ramQueueCount > 0) {
    String batchJson;
    int batchRows = 0;

    if (buildRamBatch(batchJson, FLUSH_BATCH_MAX, batchRows)) {
      if (!sendPayload(batchJson)) {
        failed = true;
      } else if (!popRamRows(batchRows)) {
        failed = true;
      } else {
        sentRam = batchRows;
  ...
  if (failed) {
    ...
    liveSendBlockedUntilMs = now + flushBackoffMs;
...
bool hasQueuedTelemetry() {
  return ramQueueCount > 0 || (sdReady && sdQueueCount > 0);
}
...
bool shouldTryLiveSend() {
  ...
  if (hasQueuedTelemetry()) return false;
...
void ensureWifiConnected() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (activeProfileIdx != wifiConnectedProfileIndex) {
...
void handleGpsSentence(const String& sentence) {
  ...
  if (triedLiveSend && sendPayload(payload)) {
    noteLiveSendOk();
