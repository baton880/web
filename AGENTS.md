...
- Host ingress разделяет конечный `pre-replay` fence и расширяемый `post-replay` catch-up. До replay обрабатывается только зафиксированное поколение history-пакетов, поэтому непрерывный live-поток не откладывает запуск навсегда. После replay fence расширяется до живого края, пока уже имеется хвост, и разбирает его FIFO: newest-live не может перепрыгнуть через пакет, пришедший сразу после исходной границы. Interrupted replay восстанавливает `post-replay` catch-up при следующем открытии ingress-БД.
...
## Production-деплой calculated replay-loop 2026-08-10

- Перед первым переключением созданы согласованные SQLite-копии и архив `/opt/backups/farm-site/pre-deploy-replay-fix-20260810_061648.tar.zst`; SHA-256 архива `da0128805b6b910ad40dc4253b1c31640d08647d91e213e76d630e59d9e0652c`. Перед catch-up hotfix создан второй архив `/opt/backups/farm-site/pre-catchup-hotfix-20260810_070716.tar.zst`; SHA-256 `65852890351252512ec5a3356e6299037a2459e8e1f9b415dd43836a99e172b0`. В обоих случаях main, HOST ingress и RTK ingress прошли `PRAGMA integrity_check = ok`, архивы проверены через `zstd -t` и чтение tar.
- Production обновлён fast-forward с `1f0f56b` до `35f56d5`, а затем до `3339164`. Первый runtime rollout выявил реальный пограничный случай: после replay fence заканчивался на packet `458067`; прибывшие во время FIFO-разгрузки `458068` и `458069` оказались за fence, обычный newest-live приоритет обработал `458069` раньше `458068` и создал новый dirty. Автоматический replay был немедленно отключён через PM2 без остановки ingress, dirty сохранён.
- Commit `3339164` добавил разные режимы fence: конечный `pre-replay` и расширяемый до живого края `post-replay`. Регрессионный тест воспроизводит поступление ещё двух пакетов уже после фиксации исходного replay fence и проверяет порядок `1..6`, current на шестом пакете, отсутствие dirty и второго replay. После изменения прошли все десять локальных наборов тестов, включая ingress/current/RTK и replay safety.
- После hotfix scheduler снова включён и сохранён в PM2 (`RTK_BUFFER_REPLAY_ENABLED=1`). Реальный day replay `2026-08-10` выполнялся `07:13:55.789–07:14:07.046 UTC`, то есть 11,3 секунды. Монитор увидел `catchupMode=post-replay`, затем пять минут непрерывного live-потока без второго запуска: replay `idle`, `queued=false`, dirty/fence/mode очищены, HOST pending/history равны нулю, writer admission открыт.
- Финальная production-проверка: public `/` и `/api/health` отвечают HTTP 200; PM2 `farm-server` online на commit `3339164`; main, HOST ingress и RTK ingress снова прошли `integrity_check = ok`. `DeviceCurrentTelemetry` и последняя HOST `Telemetry` совпали на packet `458852` с timestamp `2026-08-10 07:20:26 UTC`. После hotfix новых P1008/P2028 нет; error-log не менялся после `06:26:01 UTC`, то есть старые записи относятся к работе до hotfix.

## HOST UTC timestamp hotfix и production-деплой 2026-08-22

- Причина: Raspberry Pi отправляла HOST UTC timestamp без суффикса зоны (`2026-08-21T17:48:10`), а production Node работал в `Europe/Moscow` и интерпретировал строку как локальную MSK. В результате `Telemetry.timestamp` записывался на 3 часа раньше абсолютного времени. RTK route не затрагивался.
- Commit `29833a4` трактует только ISO date-time HOST без зоны как UTC; явные `Z` и numeric offset остаются авторитетными. Та же нормализация используется live/current/history, main database processing, out-of-order detection, replay dirty markers и FSM. Все пользовательские форматтеры и серверные telemetry warnings закреплены на `Asia/Barnaul` (Бийск, UTC+7).
- Добавлен идемпотентный `scripts/repair-host-naive-utc-timestamps.mjs`: apply требует отдельный backup, проверяет его `integrity_check` и изменяет только строки с zone-less raw timestamp и доказанным сдвигом ровно 3 часа. Отдельные unit/integration тесты проверяют UTC parsing, бийский farm day/display, selective repair, повторный dry-run, host current/ingress и replay safety.
- Перед переключением production был на `5d23f31` с локальной deployment-правкой `server/src/index.js` (`127.0.0.1:3001`); правка сохранена и не попала в Git. Согласованные backups и её patch находятся в `/opt/backups/farm-site/host-time-hotfix-20260821_181500`; main, HOST ingress и RTK ingress copies прошли `integrity_check = ok`, checksums записаны в `SHA256SUMS`, source/config archive прошёл `zstd -t`.
- Production обновлён fast-forward до `29833a4`; `npm ci`, Prisma generate/validate/migrate deploy (pending migrations: 0), timestamp/repair/raw-weight/host-ingress/replay-safety tests и JS syntax check прошли. Известные npm audit результаты не менялись: 1 low, 1 moderate, 5 high; автоматический audit fix не запускался.
- Production dry-run нашёл 16582 строки `Telemetry` (`id 1204029..1220610`) со сдвигом +3 часа по raw UTC. Все 16582 исправлены одной транзакцией; повторный dry-run вернул 0 кандидатов, `integrity_check = ok`. Затронутые farm days `2026-08-21` и `2026-08-22` точечно replay-нуты: текущий итог остался 202 замеса, 1317 компонентов, 557 нарушений, 0 открытых замесов.
- После restart реальные zone-less пакеты сохраняются с offset 0; контрольный `2026-08-21 18:17:54 UTC` отображается как `2026-08-22 01:17:54` в Бийске. Public `/` отвечает HTTP 200 и отдаёт `dashboard.js?v=20260822a`; PM2 `farm-server` online, новых записей в error log после restart нет.

## Weight filtration lab — локально 2026-08-24

- Согласованный online-backup production SQLite создан в `/opt/backups/farm-site/weight-analysis-20260824_091500`; чистая локальная копия находится в `C:\Users\baton\AppData\Local\Temp\farm-weight-20260824\server-dev.db`. SHA-256: `67499c55572d23b3ef123d7125b0a36f459ad8381349668b2deaca410ea89c85`, `PRAGMA quick_check = ok`. Production-БД и snapshot не изменялись.
- `server/scripts/generate-weight-filter-lab.mjs` формирует `server/tmp/weight-filter-lab.html` по 28 замесам за farm days 2026-08-22..2026-08-24 с пятиминутным контекстом. Даты можно переопределить через `WEIGHT_LAB_FROM` и `WEIGHT_LAB_TO`.
- Lab независимо считает от `rawWeight` некаузальный offline-кандидат (centered Hampel, median, average, rounding) и causal real-time-кандидат (trailing Hampel, trailing median, normal/fast EMA, reset on gap, rounding). Все параметры меняются в браузере без записи в SQLite; выбранная конфигурация выводится готовым JSON.
- В интерфейсе доступны raw, сохранённый `Telemetry.weight`, оба кандидата, сохранённые границы компонентов, скорость, метрики p95 изменения и масштабирование колесом. Настройки сохраняются локально в браузере, кнопка сброса возвращает исходный production offline и стартовый экспериментальный real-time preset.
- Product-фильтрация, API и главная страница не менялись: преждевременная пробная интеграция полностью удалена. Пользовательская директория `migration/` не изменялась.
- Lab проверена в браузере: 28 вариантов в трёх группах дат, смена замеса и независимый пересчёт параметров/JSON работают, canvas отрисован, console errors отсутствуют. Для отдельного запуска используется `server/scripts/serve-weight-filter-lab.mjs`; на время проверки обычного сайта lab-сервер остановлен.

## Выбранные весовые фильтры — локальная проверка 2026-08-24

- Offline postprocessing настроен на `rawWeight`: centered Hampel `radius=25, sigma=0.8`, centered median `radius=12`, average `radius=0`, округление 5 кг.
- Отдельный real-time фильтр настроен causal: trailing Hampel `window=9, sigma=1`, trailing median `window=7`, EMA `alpha=0.1`, fast EMA `alpha=0.8` при отклонении от 180 кг, reset после разрыва более 4 секунд, округление 5 кг. Он использует последние live-пакеты durable HOST ingress и не добавляет записи в main SQLite.
- Новая рабочая БД: `C:\Users\baton\AppData\Local\Temp\farm-weight-20260824\weight-selected-20260824.db`, создана заново из чистого snapshot. Перепроцессированы 28 завершённых замесов за 22–24 августа: 196 -> 160 компонентов, 72750 -> 70750 кг. Raw-счётчики сохранены (`Telemetry=1200867`, `RtkTelemetry=227705`), `PRAGMA quick_check = ok` до и после обработки.
- Локальный сайт запущен на `http://127.0.0.1:3000` с этой БД, отдельными ingress-БД, `DATA_RETENTION_ENABLED=false` и `RTK_BUFFER_REPLAY_ENABLED=0`. Product deployment, production-БД и чистый snapshot не изменялись.

## GPS fallback внутри коровников — локально 2026-08-24

- Для HOST добавлен sticky barn fallback: валидной точкой входа считается положение внутри активной зоны коровника либо не далее 5 м от её границы. При последующих невалидных координатах во время активного замеса логика использует центр запомненной зоны до 30 минут; исходная `Telemetry` с `0,0` не переписывается.
- После первого пропадания GPS зона фиксируется до завершения замеса, поэтому короткий ошибочный возврат валидных координат вне коровника не стирает память и следующий нулевой участок снова получает ту же зону. Валидная точка в другом коровнике обновляет фиксацию. Это исправило реальный замес 1247, где между двумя нулевыми участками GPS кратко прыгнул наружу.
- То же правило применяется в live HOST processing, `/api/telemetry/host/current`, на главной карте и в day replay. Для невалидного GPS `/current` отдаёт центр зоны с `positionSource=barn_sticky`; интерфейс подписывает оценочную позицию и не выдаёт её за настоящий GPS fix.
- Экспериментальная БД: `C:\Users\baton\AppData\Local\Temp\farm-weight-20260824\weight-sticky-barn-5m-20260824.db`. За 22–24 августа неизвестные группа/рацион уменьшились с 4 до 1; неизвестных замесов с компонентами — с 3 до 0. Единственный оставшийся замес 1261 пустой: 37 секунд, 0 компонентов и 0 кг.
- Raw-счётчики сохранены (`Telemetry=1200867`, `RtkTelemetry=227705`), `PRAGMA quick_check = ok`; итоговые таблицы: 230 замесов, 1477 компонентов, 640 нарушений. Пройдены sticky-helper, HOST ingress/current и replay safety tests.
- Локальный сайт запущен на `http://127.0.0.1:3000` с этой БД, отдельными ingress-БД, `DATA_RETENTION_ENABLED=false` и `RTK_BUFFER_REPLAY_ENABLED=0`; stderr пуст. Production и чистый snapshot не изменялись.

## Визуальная очистка HOST GPS-трека — локально 2026-08-24

- Очистка применяется только при отрисовке HOST-маршрута на главной карте. Raw `Telemetry`, effective position, зоны, замесы, replay и постпроцессинг не меняются.
- Точка не рисуется при заявленной GPS-скорости выше 30 км/ч, числе спутников 5 и меньше, невалидном/устаревшем fix либо расчётной скорости от последней доверенной точки выше 30 км/ч.
- После любого такого разрыва фильтр ждёт 3 последовательные нормальные точки с физически возможными переходами. После подтверждения эти точки добавляются в маршрут, а от последней доверенной позиции строится обычный администраторский пунктир; подозрительные координаты в пунктир не входят.
- На данных farm day 2026-08-24: 28564 входных строки, 27133 отрисованные точки, 1377 невалидных, 12 с GPS speed >30, 36 с <=5 спутниками и 2 дополнительных невозможных скачка; сформировано 11 восстановленных разрывов. Все 11 точек эпизода 11:05:11–11:05:50 +07 со скриншота исключены, максимальная скорость внутри сплошного сегмента — 27.43 км/ч.
- Тест `npm run test:host-track-visual-filter` покрывает speed/satellite/implied-speed отбраковку, трёхточечную стабилизацию и нормальный двухминутный дорожный GPS-разрыв. Локальный сайт запущен на `http://127.0.0.1:3000`, health/root/filter script отвечают HTTP 200, stderr пуст.
